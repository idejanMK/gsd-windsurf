...
Training data is 6-18 months stale. Always verify.

1. **Context7 MCP FIRST** — Current docs, no hallucination
2. **Official docs** — When Context7 lacks coverage
3. **WebSearch LAST** — For comparisons and trends only

See GSD_HOME/templates/discovery.md `<discovery_protocol>` for full protocol.
</source_hierarchy>
...
Route to appropriate level below.

## Level 1: Quick Verification (2-5 minutes)

For: Single known library, confirming syntax/version still correct.

[TOOL HARNESS: mcp0_resolve-library-id, mcp0_query-docs]

1. Resolve library in Context7:
   - mcp0_resolve-library-id with libraryName: "{library}"

2. Fetch relevant docs:
   - mcp0_query-docs with libraryId: {from step 1}, query: {specific concern}
...
[TOOL HARNESS: mcp0_resolve-library-id, mcp0_query-docs, search_web, read_url_content]
...
2. **Context7 for each option:**
   - mcp0_resolve-library-id for each library/framework
   - mcp0_query-docs (use "code" for API, "info" for concepts)
...
6. **Create DISCOVERY.md** using GSD_TEMPLATES/discovery.md structure:
...
1. **Scope the discovery** using GSD_TEMPLATES/discovery.md:
   - Define clear scope
...
6. **Create comprehensive DISCOVERY.md:**
   - Full structure from GSD_TEMPLATES/discovery.md
...
- options:
  - label: "Dig deeper" — description: Do more research before planning
  - label: "Proceed anyway" — description: Accept uncertainty, plan with caveats
  - label: "Pause" — description: I need to think about this

**If confidence is MEDIUM:**
Inline: "Discovery complete (medium confidence). {brief reason}. Proceed to planning?"

**If confidence is HIGH:**
Proceed directly. Note: "Discovery complete (high confidence)."
